...
/**
 * Servlet implementation class moushikomiKyusyokuListServlet
 */
@WebServlet("/moushikomiKyusyokuListServlet")
public class moushikomiKyusyokuListServlet extends HttpServlet {
     ...
     */
    public moushikomiKyusyokuListServlet() {
        super();
	...
	private void perform(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//Su dung parameter kyusyokusya_id de truyen vao function selectAll, dung lam dk get data
		request.setCharacterEncoding("UTF-8");
		String userId = request.getParameter("reqValue");
		kyusyokusyaBean ksbean = new kyusyokusyaBean();
		ksbean = moushikomiKyusyokuListServices.selectKyusyokusyaId(userId);
		int kyusyokusyaId = ksbean.getKyusyokusyaId();
    	...
    	if(actString.equals("show")) {
    		...
    		list=moushikomiKyusyokuListServices.selectAll(kyusyokusyaId);
    		JsonElement element = gson.toJsonTree(list, new TypeToken<List<KyusyokuListFetchModel>>(){
    		}.getType());
